...
namespace Com.TheFallenGames.OSA.Core
{
        ...
        where TItemViewsHolder : BaseItemViewsHolder
    {
        ...
        [SerializeField] protected TParams _Params = null;
        ...
        {
            get { return _Params.IsHorizontal; }
        }
        ...
        {
            get { return _Params.Content; }
        }

        public RectTransform Viewport
        {
            get { return _Params.Viewport; }
        }

        /// <inheritdoc/>
        double IScrollRectProxy.ContentInsetFromViewportStart
        {
            get { return ContentVirtualInsetFromViewportStart; }
        }

        /// <inheritdoc/>
        double IScrollRectProxy.ContentInsetFromViewportEnd
        {
            get { return ContentVirtualInsetFromViewportEnd; }
        }
        ...
        {
            get { return _InternalState.ctVirtualInsetFromVPS_Cached; }
        }

        /// <inheritdoc/>
        public double ContentVirtualInsetFromViewportEnd
        {
            get { return _InternalState.CTVirtualInsetFromVPE_Cached; }
        }
        ...
        {
            get { return _VisibleItemsCount; }
            internal set { _VisibleItemsCount = value; }
        }
        ...
        {
            get { return _RecyclableItems.Count; }
        }
        ...
        public int BufferedRecyclableItemsCount
        {
            get { return _BufferredRecyclableItems.Count; }
        }
        ...
        public virtual bool InsertAtIndexSupported
        {
            get { return true; }
        }
        ...
        public virtual bool RemoveFromIndexSupported
        {
            get { return true; }
        }
        ...
        public float Time
        {
            get { return _Params.UseUnscaledTime ? UnityEngine.Time.unscaledTime : UnityEngine.Time.time; }
        }
        ...
        {
            get { return _Params.UseUnscaledTime ? UnityEngine.Time.unscaledDeltaTime : UnityEngine.Time.deltaTime; }
        }
        ...
        protected internal List<TItemViewsHolder> _RecyclableItems;

        /// <summary>Second choice from where to extract new items</summary>
        protected internal List<TItemViewsHolder> _BufferredRecyclableItems;
        ...
        {
            if (IsInitialized) MyUpdate();
        }

        protected virtual void LateUpdate()
        {
            if (IsInitialized) MyLateUpdate();
        }

        protected virtual void OnDisable()
        {
            if (IsInitialized) MyOnDisable();
        }
        ...
        {
            return Math.Min(_InternalState.ctVirtualSize, float.MaxValue);
        }
 ...
 "OnInitializePotentialDrag " + name + ": " + eventData.pointerId + " " + eventData;
        ...
        public virtual void OnDrag(PointerEventData eventData)
        {
            ...
            double abstrDeltaInCTSpace =
                _InternalState.GetCTAbstractSpaceVectorLongitudinalComponentFromCTSpaceVector(ctSpaceDelta);
            //Debug.Log(
            ...
            var alllowOutOfBoundsMode =
                (_Params.effects.ElasticMovement ||
                 _Params.effects.LoopItems && _InternalState.VirtualScrollableArea > 0d)
            ...
            if (_Params.effects.Inertia)
            {
                ...
                if (_LastOnDragPointerPosInCTSpace == _OnBeginDragPointerPosInCTSpace)
                {
                    ...
                    var ctSpaceDeltaForFirstDragForVelocity =
                        pointerPosInCTSpace - _OnInitializePotentialDragPointerPosInCTSpace;
                    abstrDeltaToUseForVelocity =
                        _InternalState.GetCTAbstractSpaceVectorLongitudinalComponentFromCTSpaceVector(
                            ctSpaceDeltaForFirstDragForVelocity);
                }
                else
                {
                ...
                _Velocity[_InternalState.hor0_vert1] = Mathf.Clamp((float)((velocityBef + velocityThisFrame) / 2d),
                    -_Params.effects.MaxSpeed, _Params.effects.MaxSpeed);
            }
        ...
        public virtual void OnEndDrag(PointerEventData eventData)
        {
            ...
            if (_Params.effects.Inertia)
            {
                ...
                float inertiaWaitTimeWindow =
                    Mathf.Max(DeltaTime, .1f); // max time to wait for the inertia to still be applied

                if (dtSinceLastOnDragStrictlyPositive > inertiaWaitTimeWindow)
                    zeroVelocity = true;
                else
                {
                    var ctSpaceDeltaFromPressPosition = _InternalState.GetPointerPositionInCTSpace(eventData) -
                                                        _OnInitializePotentialDragPointerPosInCTSpace;
                    double abstrDeltaFromPressPositionInCTSpace =
                        _InternalState.GetCTAbstractSpaceVectorLongitudinalComponentFromCTSpaceVector(
                            ctSpaceDeltaFromPressPosition);
                ...
                else
                {
                    ...
                    {
                        _Velocity[_InternalState.hor0_vert1] = Mathf.Clamp(
                            (curVelocity + velocityToAdd) * (1 + _StackedDragSpeeds), -_Params.effects.MaxSpeed,
                            _Params.effects.MaxSpeed);
        ...
        public void Init()
        {
            if (!enabled || !gameObject.activeInHierarchy)
                throw new OSAException(
                    "Init should only be called when the GameObject is active in hierarchy and the OSA component is enabled");
            ...
            _RecyclableItems = new List<TItemViewsHolder>();
            _BufferredRecyclableItems = new List<TItemViewsHolder>();
        ...
        {
            ChangeItemsCount(ItemCountChangeMode.RESET, _ItemsDesc.itemsCount, -1, contentPanelEndEdgeStationary,
                keepVelocity);
        }
        ...
        public virtual void InsertItems(int index, int itemsCount, bool contentPanelEndEdgeStationary = false,
            bool keepVelocity = false)
        {
            ChangeItemsCount(ItemCountChangeMode.INSERT, itemsCount, index, contentPanelEndEdgeStationary,
                keepVelocity);
        }
        ...
        public virtual void RemoveItems(int index, int itemsCount, bool contentPanelEndEdgeStationary = false,
            bool keepVelocity = false)
        {
            ChangeItemsCount(ItemCountChangeMode.REMOVE, itemsCount, index, contentPanelEndEdgeStationary,
                keepVelocity);
        }
        ...
        public virtual void ChangeItemsCount(ItemCountChangeMode changeMode, int itemsCount,
            int indexIfInsertingOrRemoving = -1, bool contentPanelEndEdgeStationary = false, bool keepVelocity = false)
        {
            ChangeItemsCountInternal(changeMode, itemsCount, indexIfInsertingOrRemoving, contentPanelEndEdgeStationary,
                keepVelocity, false);
        }
        ...
        )
        {
            ...
            return GetViewsHolderClosestToViewportLongitudinalNormalizedAbstractPoint(c, canvasRectTransform,
                posSelected, itemNormalizedAbstractPoint, out distance) as TItemViewsHolder;
        }
        ...
        /// </summary>
        public virtual void RemoveItemWithViewsHolder(TItemViewsHolder viewsHolder,
            bool stealViewsHolderInsteadOfRecycle, bool contentPanelEndEdgeStationary)
        {
            ChangeItemsCountInternal(ItemCountChangeMode.REMOVE, 1, viewsHolder.ItemIndex,
                contentPanelEndEdgeStationary, false, stealViewsHolderInsteadOfRecycle);
            if (stealViewsHolderInsteadOfRecycle)
            {
                if (!viewsHolder.root)
                    throw new OSAException("RemoveItemWithViewsHolder: Internal error: vh.root destroyed. ItemIndex " +
                                           viewsHolder.ItemIndex + ", viewsHolder.root " + viewsHolder.root);
            }
        }
        ...
        /// </summary>
        public virtual void InsertItemWithViewsHolder(TItemViewsHolder viewsHolder, int atIndex,
            bool contentPanelEndEdgeStationary)
        {
            if (atIndex < 0 || atIndex > _ItemsDesc.itemsCount)
                throw new ArgumentOutOfRangeException("atIndex", atIndex,
                    "should be >=0 and <= itemsCount(=" + _ItemsDesc.itemsCount + ")");

            if (!viewsHolder.root)
                throw new ArgumentOutOfRangeException(
                    "Could not give a viewsholder that has its root destroyed/set to null");
            ...
            ChangeItemsCountInternal(ItemCountChangeMode.INSERT, 1, atIndex, contentPanelEndEdgeStationary, false,
                false);
        }
        ...
        public virtual void ScrollTo(int itemIndex, float normalizedOffsetFromViewportStart = 0f,
            ...
            bool overrideCurrentScrollingAnimation = false)
        {
            ...
            _SmoothScrollCoroutine = StartCoroutine(SmoothScrollProgressCoroutine(itemIndex, duration,
                normalizedOffsetFromViewportStart, normalizedPositionOfItemPivotToUse, onProgress, onDone));
        ...
        public virtual void BringToView(int itemIndex, float? spacingFromViewportEdge = null)
        {
            ...
            if (!BringToView_CalculateParameters(itemIndex, spacingFromViewportEdge,
                    out normalizedOffsetFromViewportStart, out itemPivotToUse))
                return;
        ...
        public virtual bool SmoothBringToView(int itemIndex, float duration, float? spacingFromViewportEdge = null,
            Func<float, bool> onProgress = null, Action onDone = null, bool overrideCurrentScrollingAnimation = false)
        {
            float normalizedOffsetFromViewportStart, itemPivotToUse;
            if (!BringToView_CalculateParameters(itemIndex, spacingFromViewportEdge,
                    out normalizedOffsetFromViewportStart, out itemPivotToUse))
            ...
            return SmoothScrollTo(itemIndex, duration, normalizedOffsetFromViewportStart, itemPivotToUse, onProgress,
                onDone, overrideCurrentScrollingAnimation);
        }

        internal bool BringToView_CalculateParameters(int itemIndex, float? spacingFromViewportEdge,
            out float normalizedOffsetFromViewportStart, out float itemPivotToUse)
        {
        ...
        public virtual double GetItemSignedVisibility(int itemIndex, float? spacingFromViewportEdge = null)
        {
            ...
            double startInset = visible
                ? GetItemRealInsetFromParentStart(vhIfVisible.root)
                : _InternalState.GetItemInferredRealInsetFromParentStart(indexInView);
            double startInsetAdjusted =
                startInset - spacingToUse; // calculating visibility relative to vp's start+spacing
            ...
            {
                double endInset = visible
                    ? GetItemRealInsetFromParentEnd(vhIfVisible.root)
                    : _InternalState.GetItemInferredRealInsetFromParentEnd(indexInView);
                double endInsetAdjusted =
                    endInset - spacingToUse; // calculating visibility relative to vp's end+spacing
        ...
        public float RequestChangeItemSizeAndUpdateLayout(TItemViewsHolder withVH, float requestedSize,
            bool itemEndEdgeStationary = false, bool computeVisibility = true, bool correctItemPosition = false,
            bool keepVelocity = false)
        {
            return RequestChangeItemSizeAndUpdateLayout(withVH.ItemIndex, requestedSize, itemEndEdgeStationary,
                computeVisibility, correctItemPosition, keepVelocity);
        }
        ...
        {
            return _InternalState.GetItemVirtualInsetFromParentStartUsingItemIndexInView(
                _ItemsDesc.GetItemViewIndexFromRealIndexChecked(itemIndex));
        }
        ...
        public double SetVirtualAbstractNormalizedScrollPosition(double pos, bool computeVisibilityNow, out bool looped,
            bool keepVelocity = false)
        {
            CancelAllAnimations();
        ...
        public void ForceUpdateVisibleItems()
        {
            ...
            if (twinPassScheduledBefore)
                throw new OSAException(
                    "You shouldn't call ForceUpdateVisibleItems during a ComputeVisibilityForCurrentPosition, UpdateViewsHolder or CreateViewsHolder");
            //TODO: 这里为什么VisibleItemsCount - 1 ??最后一个可见的item不需要更新吗？
        ...
        public bool ForceUpdateViewsHolderIfVisible(int itemIndex)
        {
            ...
            if (twinPassScheduledBefore)
                throw new OSAException(
                    "You shouldn't call ForceUpdateViewsHolderIfVisible during a ComputeVisibilityForCurrentPosition, UpdateViewsHolder or CreateViewsHolder");
        ...
        protected virtual void CollectItemsSizes(ItemCountChangeMode changeMode, int count,
            int indexIfInsertingOrRemoving, ItemsDescriptor itemsDesc)
        {
            ...
            itemsDesc.ReinitializeRealIndexOfFirstItemInView(oldCount, changeMode, count, indexIfInsertingOrRemoving,
                itemIndexOfFirstVH, _Params.effects.LoopItems);
        ...
        protected virtual void OnItemIndexChangedDueInsertOrRemove(TItemViewsHolder shiftedViewsHolder, int oldIndex,
            bool wasInsert, int removeOrInsertIndex)
        {
        }
        ...
        protected virtual bool IsRecyclable(TItemViewsHolder potentiallyRecyclable,
        ...
        internal void InternalDestroyRecyclableItem(TItemViewsHolder vh, bool isVisible)
        {
            ...
            {
                #if UNITY_EDITOR
                GameObject.DestroyImmediate(vh.root.gameObject);
                #else
        ...
        internal void InternalOnBeforeRecycleOrDisableViewsHolder(TItemViewsHolder inRecycleBinOrVisible,
            int newItemIndex)
        {
        ...
        protected virtual void OnBeforeRecycleOrDisableViewsHolder(TItemViewsHolder inRecycleBinOrVisible,
            int newItemIndex)
        {
        ...
        protected void ScheduleComputeVisibilityTwinPass(bool preferContentEndEdgeStationaryIfSizeChanges = false)
        {
            ...
            _InternalState.preferKeepingCTEndEdgeStationaryInNextComputeVisibilityTwinPass =
        ...
        /// </summary>
        protected void ForceRebuildViewsHolderAndUpdateSize(TItemViewsHolder viewsHolder,
            bool itemEndEdgeStationary = false, bool correctPosition = true)
        {
        ...
        )
        {
            ...
            {
                var v = UIUtils8.Instance.GetWorldVectorBetweenCustomLocalPivots(_Params.Viewport,
                    viewportPointNormalized, vh.root, itemPointNormalized, c, canvasRectTransform);
        ...
        )
        {
            ...
            return GetViewsHolderClosestToViewportNormalizedPoint(
                viewsHolders, c, canvasRectTransform, viewportPointNormalizedWithYInverted,
                itemPointNormalizedWithYInverted, out distance
            );
        }
        ...
        protected virtual void OnApplyCustomGalleryEffects(TItemViewsHolder vh, double itemCenterDistFromStart01,
