...
namespace Com.TheFallenGames.OSA.CustomAdapters.GridView
{
	...
	public class GridParams : BaseParams
	{
		...
		{
			var name = GetCellGroupPrefabName(ScrollViewRT.name);
			var go = new GameObject(name, typeof(RectTransform));
			return go;
		}
		
		//如果以w_开头,将其去除掉(防止LightenUI自动生成) add by ggyy 
		public string GetCellGroupPrefabName(string parentName)
		{
			var name = parentName + "_CellGroupPrefab";
...
#if UNITY_EDITOR
				//osa预览功能 add by ggyy 2023.5.5
				_TheOnlyGroupPrefabAsHorizontalOrVertical.childControlWidth = true;
				_TheOnlyGroupPrefabAsHorizontalOrVertical.childControlHeight = true;
#endif
			}
