...
public class RotationGear: GearBase
{
    private RectTransform _target;

    private Vector3[] _values;

    public RotationGear(Controller parent, GearConfig config): base(parent, config)
    {
    }
    ...
    public override void Init(GearConfig config)
    {
        ...
        {
            _values[(i - 1) / 3] = new Vector3(Owner.StoredFloats[config.dataArray[i]],
    ...
    {
        base.Apply();
        _target.localEulerAngles = _values[_parent.SelectedIndex];
    }
}