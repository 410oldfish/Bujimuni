...
public class RawImageColorGear: GearBase
{
    private RawImage _target;

    private Color[] _values;

    public RawImageColorGear(Controller parent, GearConfig config): base(parent, config)
    {
    }
    ...
    public override void Init(GearConfig config)
    {
        ...
        _target = go.GetComponent<RawImage>();
        ...
        {
            _values[(i - 1) / 4] = new Color(Owner.StoredFloats[config.dataArray[i]],
