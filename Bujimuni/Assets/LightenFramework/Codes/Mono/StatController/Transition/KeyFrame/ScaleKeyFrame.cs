...
public class ScaleKeyFrame: ValuesKeyFrameBase
{
    private RectTransform _target;

    public ScaleKeyFrame(Transition parent, KeyFrameConfig config): base(parent, config)
    {
    }
    ...
    public override bool ApplyValue(TransitionTweenAction tweenAction)
    {
        ...
        Vector3 value = new Vector3(tweenAction.Actives[0]? tweenAction.Value[0] : _target.localScale.x,
            tweenAction.Actives[1]? tweenAction.Value[1] : _target.localScale.y,
            tweenAction.Actives[2]? tweenAction.Value[2] : _target.localScale.z);
    ...
    public override void ApplyValue()
    {
        if (_target != null)
        {
            Vector3 value = new Vector3(_actives[0]? _values[0] : _target.localScale.x,
                _actives[1]? _values[1] : _target.localScale.y,
                _actives[2]? _values[2] : _target.localScale.z);
            _target.localScale = value;
        }
    }
}