...
namespace Lighten
{
    ...
    public class XMonoVariableDrawer : OdinValueDrawer<XMonoVariable>
    {
        protected override void DrawPropertyLayout(GUIContent label)
        {
            ...
            target.type =
                (XMonoVariableType)EditorGUILayout.EnumPopup(GUIContent.none, target.type, GUILayout.Width(100));
        ...
        void DrawValueGameObject(XMonoVariable target)
        {
            EditorGUI.BeginChangeCheck();
            target.gameobject =
                EditorGUILayout.ObjectField(target.gameobject, typeof(GameObject), true, GUILayout.MinWidth(100)) as
                    GameObject;
        ...
        void DrawValueComponent(XMonoVariable target)
        {
            if (target.gameobject != null)
            {
                ...
                EditorGUI.BeginChangeCheck();
                var newIndex = EditorGUILayout.Popup(GUIContent.none, index, contents.ToArray(),
                ...
                if (target.objData == null)
                {
                    target.objData = components[0];
                    target.componentType = components[0].GetType();
                }
            }
            EditorGUI.BeginChangeCheck();
            target.gameobject =
