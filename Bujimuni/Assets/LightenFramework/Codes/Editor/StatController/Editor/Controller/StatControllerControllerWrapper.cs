...
public class StatControllerControllerWrapper
{
    ...
    public StatControllerControllerWrapper(ControllerConfig config)
    {
        RebuildTree();
    ...
    public void AnalyzeControllerConfig(ControllerConfig config)
    {
        ...
        for (int i = 0; i < config.gearConfigs.Length; i++)
        {
            ...
            GameObject target = StatControllerManager.Instance.CurStatController.GetStoredGameObject(gearConfig.StoredGameObjectIndex);
            if (target == null)
            {
                continue;
            }
            ...
            switch (gearConfig.gearType)
            {
                case GearTypeState.Controller:
                    gearTreeItem = goTreeItem.GetTargetGear(gearConfig.gearType,
                        StatControllerManager.Instance.CurStatController.StoredStrings[gearConfig.dataArray[1]]);
                    break;
                default:
                    gearTreeItem = goTreeItem.GetTargetGear(gearConfig.gearType);
                    break;
            ...
            gearTreeItem.LoadConfig(gearConfig);
        }
    }

    public StatControllerControllerWrapper(string name, List<string> pageNameList, List<string> pageTipsList)
    {
        _name = name;
    ...
    {
        if (StatControllerManager.Instance.CurStatController.gameObject == null)
        {
            return;
        }

        ControllerRootTreeItem newTreeRoot = new ControllerRootTreeItem(StatControllerManager.Instance.CurStatController.gameObject);
        Dictionary<int, ControllerGameObjectTreeItem> newGOTreeItemDic = new Dictionary<int, ControllerGameObjectTreeItem>();

        newGOTreeItemDic.Add(StatControllerManager.Instance.CurStatController.gameObject.GetInstanceID(), newTreeRoot);
        newTreeRoot.AddGearItems();

        foreach (Transform child in StatControllerManager.Instance.CurStatController.gameObject.transform)
        {
    ...
    public void AddChildToTree(Transform target, ControllerTreeItemBase parent, Dictionary<int, ControllerGameObjectTreeItem> goTreeItemDic,
    bool refresh)
    {
        ...
        foreach (GearTypeState gearType in Enum.GetValues(typeof (GearTypeState)))
        {
            if (ControllerGameObjectTreeItem.CheckCanAddGear(target.gameObject, gearType))
            {
                ...
                {
                    case GearTypeState.Controller:
                        for (int i = 0; i < target.gameObject.GetComponent<StatController>().ControllerConfigs.Length; i++)
                        {
                            if (refresh)
                            {
                                ...
                                if (_goTreeItemDic.TryGetValue(target.gameObject.GetInstanceID(), out oldGameObjectTreeItem))
                                {
                                    ControllerGearTreeItemBase oldGearTreeItem = oldGameObjectTreeItem.GetTargetGear(gearType,
                                        target.gameObject.GetComponent<StatController>().ControllerConfigs[i].name);
                            ...
                            ControllerGearTreeItem controllerGearTreeItem = new ControllerGearTreeItem(
                                target.gameObject.GetComponent<StatController>().ControllerConfigs[i].name,
                                target.gameObject.GetComponent<StatController>().ControllerConfigs[i].pageNames);
                    ...
                    default:
                        ...
                        string gearClassName = gearType.ToString() + "GearTreeItem";
    ...
    public List<GearConfig> RebuildGearConfigList(StatControllerStoredDataBuilder dataBuilder)
    {
    ...
    public void ChangeSelectedIndex(int index)
    {
        ...
        bool inRecordingState = StatControllerManager.Instance.ControllerSettings.InRecordingState;
        StatControllerManager.Instance.ControllerSettings.InRecordingState = false;
        _selectedIndex = index;
        if (StatControllerManager.Instance.ControllerSettings.AutoApply || inRecordingState)
        {
            Apply();
        }
        ...
        if (inRecordingState)
        {
            StatControllerManager.Instance.ControllerSettings.InRecordingState = true;
        }
    }

    public void ApplyEditData(List<ControllerWrapperEditAction> editActionList, string wrapperName, List<string> pageNameList,
    List<string> pageTipsList)
    {
    ...
    {
        _treeRoot.Apply();
        EditorUtility.SetDirty(StatControllerManager.Instance.CurStatController.gameObject);
    }
}