...
public class RawImageColorGearTreeItem: ControllerGearTreeItemBase
{
    ...
    public RawImageColorGearTreeItem(): base(GearTypeState.RawImageColor)
    {
    ...
    {
        Target.GetComponent<RawImage>().color = (_values[pageIndex]);
    }
    ...
    public override void LoadConfig(GearConfig config)
    {
        ...
        {
            Color color = new Color(UIScript.StoredFloats[config.dataArray[i]],
    ...
    {
        if (Target.GetComponent<RawImage>().color == _monitorCheckValue)
        {
            return;
        }
        ...
        if (stateChanged)
        {
            ...
            for (int i = 0; i < ControllerWrapper.PageNameList.Count; i++)
            {
                ...
                {
                    _values.Add(Target.GetComponent<RawImage>().color);
                }
                else
                {
        ...
        {
            _values[SelectedIndex] = Target.GetComponent<RawImage>().color;
        }

        _monitorCheckValue = Target.GetComponent<RawImage>().color;
        this.OnValueChanged();
    }
