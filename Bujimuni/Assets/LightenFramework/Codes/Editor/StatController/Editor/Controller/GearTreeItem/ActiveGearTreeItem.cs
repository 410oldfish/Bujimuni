...
public class ActiveGearTreeItem: ControllerGearTreeItemBase
{
    ...
    public override void OnDetailGUI(int index, Rect itemArea)
    {
        ...
        for (int i = 0; i < _values.Count; i++)
        {
            Rect valueArea = new Rect(
                itemArea.x + StatControllerEditorUtility.CONTROLLER_PAGE_WIDTH * i +
                (StatControllerEditorUtility.CONTROLLER_PAGE_WIDTH - EditorGUIUtility.singleLineHeight) / 2.0f,
                itemArea.y + 1,
    ...
    public override void LoadConfig(GearConfig config)
    {
        ...
        {
            _values.Add(config.dataArray[i] == StatControllerUtility.TrueValue);
        }
