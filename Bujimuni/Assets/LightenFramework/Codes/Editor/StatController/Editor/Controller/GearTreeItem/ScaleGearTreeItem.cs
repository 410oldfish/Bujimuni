...
using UnityEditor;
    ...
    public class ScaleGearTreeItem : ControllerGearTreeItemBase
    {
        ...
        public ScaleGearTreeItem() : base(GearTypeState.Scale)
        {
        ...
        {
            Target.GetComponent<RectTransform>().localScale = (_values[pageIndex]);
        }
        ...
        {
            if (Target.GetComponent<RectTransform>().localScale == _monitorCheckValue)
            {
                return;
            }
            ...
            if (stateChanged)
            {
                ...
                for (int i = 0; i < ControllerWrapper.PageNameList.Count; i++)
                {
                    ...
                    {
                        _values.Add(Target.GetComponent<RectTransform>().localScale);
                    }
                    else
                    {
            ...
            {
                _values[SelectedIndex] = Target.GetComponent<RectTransform>().localScale;
            }
            _monitorCheckValue = Target.GetComponent<RectTransform>().localScale;
            this.OnValueChanged();
        }
